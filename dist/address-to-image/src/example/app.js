...
const app = express();
app.get('/propertyPhoto/:address', controllers_1.realtorRequest);
app.get('/propertyPhoto1/:address', controllers_1.redfinRequest);
app.get('/propertyPhoto2/:address', controllers_1.truliaRequest);
...
function makeHomeSnapRequest(address) {
    ...
    return request(options).then(body => {
        ...
        try {
            homeSnapImgUrl = homeSnapImgUrl + jsonObject.d.Properties[0].Url;
        }
        catch (err) {
...
function getHomeSnapImgUrl(homeSnapUrl) {
    ...
    return request(options).then(body => {
        const imgUrl = cheerio('.listingImage large', body);
        if (!imgUrl) {
